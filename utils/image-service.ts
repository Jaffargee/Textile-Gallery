...
import type { ImageDirProps, ImageItem } from "@/types";
...
class ImageService {
      ...
      private constructImageUrls(bucketUrl: string, fileName: string) {
            ...
            const thumbnailUri = `${baseUrl}?width=400&height=400&quality=75`;
      ...
      private async _fetchImagesInternal(
            ...
            return data.map((file) => {
                  ...
                  const { url, thumbnailUri } = this.constructImageUrls(bucketUrl, filePath);
      ...
      private async _fetchImageDirInternal(
            ...
            return data.map((file) => {
                  const { url, thumbnailUri } = this.constructImageUrls(
                        bucketUrl, 
                        `${file.name}/`
                  );
