...
// router.navigate('/' + item?.dir_name as RelativePathString)
const Album = React.memo(function ImageCardThumbnailComponent({item}: ImageCardThumbnailProps){
      ...
      return (
            <View className='flex flex-col' style={{ width: ITEM_WIDTH }}>
                  <View className='overflow-hidden rounded-3xl border border-gray-100 relative bg-gray-100' style={{ width: ITEM_WIDTH, height: ITEM_WIDTH }}>
                        <Pressable className='flex-1 overflow-hidden' onPress={() => router.navigate('/' + item?.dir_name as RelativePathString)}>
                              <View className='flex-1 overflow-hidden'>
                                    <ExpoImage
                                          ...
                                          transition={100}
                                          placeholder={require('@/assets/images/image-loading.gif')}
                  ...
                  <View className='flex flex-col items-start justify-start w-full relative gap-2 px-1 py-1'>
                        <View>
                              <Text className='text-gray-700 text-md truncate flex-wrap'>{item?.dir_name}</Text>
                              <Text className='text-gray-500 text-sm'>26</Text>
                        </View>
                  </View>
            </View>
      )
