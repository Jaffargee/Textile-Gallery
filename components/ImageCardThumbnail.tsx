...
const ImageCardThumbnail = React.memo(function ImageCardThumbnailComponent({selected, item, style, onView, onSelect}: CardThumbnailProps){
      ...
      return (
            <View className='overflow-hidden rounded-sm bg-gray-200' style={[{ width: ITEM_WIDTH + 2, height: ITEM_WIDTH }, style]}>
                  <Pressable onPress={() => onSelect?.(item.source?.uri as string)} className='flex-1 overflow-hidden'>
                        ...
                        {
                              selected && selected?.filter(u => u === item.source?.uri).length > 0 &&
